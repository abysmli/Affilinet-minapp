...
function getExtentInfo() {
	return {
		...
		r4: "wx",
		ext: "v=" + MTA_CONFIG.version + (null !== MTA.Data.userInfo ? ";ui=" + JSON.stringify(MTA.Data.userInfo) : "")
	}
}
var MTA = {
	App: {
	...
	Page: {
		...
		stat: function() {
			if("" != MTA_CONFIG.app_id) {
				for(var a = [], b = 0, e = [getMainInfo(), getExtentInfo(), getBasicInfo(), {
						rand: +new Date
					}], d = e.length; b < d; b++)
					for(var f in e[b]) e[b].hasOwnProperty(f) && a.push(f + "=" + ("undefined" == typeof e[b][f] ? "" : e[b][f]));
				wx.request({
	...
	Event: {
		stat: function(a, b) {
			if("" != MTA_CONFIG.event_id) {
				...
				var k = [],
					g;
				for(g in c) c.hasOwnProperty(g) && k.push(g + "=" + c[g]);
				c = k.join(";");
				f.r5 = c;
				c = 0;
				d = [d, f, getBasicInfo(), {
				...
				for(f = d.length; c < f; c++)
					for(var h in d[c]) d[c].hasOwnProperty(h) && e.push(h + "=" + ("undefined" == typeof d[c][h] ? "" : d[c][h]));
				wx.request({
